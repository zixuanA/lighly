...
class SongRecyclerViewAdapter(private var messageBean: MessageBean) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun getItemCount(): Int = messageBean.result.size +1
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if(position != itemCount-1){
            ...
            if(position == 0){
                (holder.itemView.layoutParams as RecyclerView.LayoutParams).apply { topMargin = dp2px(context!!,40) }
            }else{
                (holder.itemView.layoutParams as RecyclerView.LayoutParams).apply { topMargin = dp2px(context!!,0) }
            }

            holder.itemView.img_recycler_item_song_music_playing.visibility
            Glide.with(context!!).load(messageBean.result[position].pic).into(holder.itemView.img_recycler_item_song_music_img)
